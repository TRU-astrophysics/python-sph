...
def newton_smoothlength_while(j, position_arr, initial_smoothlength_j):
    ...
    while i < NEWTON_ITERATION_LIMIT:
        ...
        if smoothlength_variation(old_smoothlength_j,
                                  current_smoothlength_j) < SMOOTHLENGTH_VARIATION_TOLERANCE and current_smoothlength_j > 0:
...
def energy_evolve(j, position_arr, velocity_arr, energy_arr, pressure_arr, density_arr, smoothlength_j, dt):
    ...
    for i in range(velocity_arr.shape[0]):
        density_change += PARTICLE_MASS * np.dot(velocity_arr[j] - velocity_arr[i],
                                                 dellM4(position_arr[j], position_arr[i], smoothlength_j))
...
def grav_potential(dist, smoothlength):
    ...
    return np.piecewise(x,
                        ...
                        [1 / smoothlength * (2 / 3 * x ** 2 - 3 / 10 * x ** 3 + 1 / 10 * x ** 5 - 7 / 5),
                         1 / smoothlength * (4 / 3 * x ** 2 - x ** 3 + 3 / 10 * x ** 4 - 1 / 30 * x ** 5 - 8 / 5 + 1 / (
                                 15 * x)),
                         -1 / dist])
...
def grav_force(dist, smoothlength):
    ...
    return np.piecewise(x,
                        ...
                        [1 / (smoothlength ** 2) * (4 / 3 * x - 6 / 5 * x ** 3 + 1 / 2 * x ** 4),
                         1 / (smoothlength ** 2) * (
                                 8 / 3 * x - 3 * x ** 2 + 6 / 5 * x ** 3 - 1 / 6 * x ** 4 - 1 / (15 * x ** 2)),
                         1 / (dist ** 2)])
...
def smoothed_gravity_correction_comp(j, i, position_arr, smoothlength_arr, omega_arr, xi_arr):
    return -G / 2 * PARTICLE_MASS * (
            xi_arr[j] / omega_arr[j] * dellM4(position_arr[j], position_arr[i], smoothlength_arr[j])
...
def fluid_acceleration_comp(j, i, position_arr, density_arr, pressure_arr, smoothlength_arr, omega_arr):
    ...
    return -PARTICLE_MASS * (
            pressure_arr[j] / (omega_arr[j] * density_arr[j] ** 2) * dellM4(position_arr[j], position_arr[i],
                                                                            smoothlength_arr[j])
            + pressure_arr[i] / (omega_arr[i] * density_arr[i] ** 2) * dellM4(position_arr[j], position_arr[i],
                                                                              smoothlength_arr[i]))


# Acceleration
def fluid_accel_viscosity_comp(j, i, position_arr, velocity_arr, density_arr, pressure_arr, smoothlength_arr,
                               omega_arr):
    if np.array_equal(position_arr[j], position_arr[i]):
        return 0
...
def acceleration(j, position_arr, density_arr, velocity_arr, pressure_arr, smoothlength_arr, omega_arr, xi_arr):
    ...
    for i in range(position_arr.shape[0]):
        acc += (fluid_accel_viscosity_comp(j, i, position_arr, velocity_arr, density_arr, pressure_arr,
                                           smoothlength_arr, omega_arr)
...
def acceleration_arr(position_arr, density_arr, velocity_arr, pressure_arr, smoothlength_arr, omega_arr, xi_arr):
    ...
    for i in range(position_arr.shape[0]):
        acc_arr[i] = acceleration(i, position_arr, density_arr, velocity_arr, pressure_arr, smoothlength_arr, omega_arr,
                                  xi_arr)
...
def static_smoothlength_sim(time, positions_with_time, velocities_with_time, energies_with_time):
    ...
    for t in range(len(time) - 1):
        positions_with_time[:, :, t + 1], velocities_with_time[:, :, t + 1], energies_with_time[:,
                                                                             t + 1] = static_smoothlength_leapfrog(
            positions_with_time[:, :, t],
            velocities_with_time[:, :, t],
            energies_with_time[:, t],
            dt)
